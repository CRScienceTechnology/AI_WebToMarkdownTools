...
class GitBookTransformerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('GitBook to Markdown Converter')
        self.setGeometry(100, 100, 800, 600)

        self.text_edit = QTextEdit(self)
        self.text_edit.setPlaceholderText("Paste your GitBook content here...")

        self.convert_button = QPushButton('Convert to Markdown', self)
        self.convert_button.clicked.connect(self.convert_to_markdown)
    ...
    def convert_to_markdown(self):
        gitbook_content = self.text_edit.toPlainText()
        markdown_content = transform_gitbook_to_markdown(gitbook_content)
        self.text_edit.setPlainText(markdown_content)


def transform_gitbook_to_markdown(gitbook_content):
    """
    将 GitBook 内容转换为 Markdown。
    """
    soup = BeautifulSoup(gitbook_content, 'html.parser')
